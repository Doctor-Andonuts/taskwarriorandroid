...
public class EditorActivity extends AppCompatActivity {
    ...
    private String save() {
        ...
        Boolean needsInboxTag = false;
        Boolean hasContextTags = false;
        ...
        for (String key : form.changes()) { // Make changes
            ...
            if (!priorities.get(form.getValue(App.KEY_EDIT_PRIORITY, Integer.class)).matches("H|M|L")) {
                needsInboxTag = true;
            }

            // Normally we would check to see if tags have changed at all, and only submit tags: to
            // the command if they changed.  But since we can dynamically add inbox tag, we always
            // have to process tags.
            String tagsStr = form.getValue(App.KEY_EDIT_TAGS);
            if (tagsStr.contains("computer") || tagsStr.contains("errand") || tagsStr.contains("home") || tagsStr.contains("outside")) {
                hasContextTags = true;
            }
